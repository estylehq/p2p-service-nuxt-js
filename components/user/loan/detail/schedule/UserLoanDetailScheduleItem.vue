    ...
    <b-col cols="12" class="non-padding">
      <b-collapse :id="modalId">
        <div class="detail">
          <b-row>
            <b-col>
              {{ $t('user.loan.detail.schedule.principal') }}
            </b-col>
            <b-col>{{ $n(principal, 'currency') }}</b-col>
          </b-row>
          <b-row>
            <b-col>
              {{ $t('user.loan.detail.schedule.interest') }}
            </b-col>
            <b-col>{{ $n(interest, 'currency') }}</b-col>
          </b-row>
          <b-row v-if="lateFee > 0">
            <b-col>
              {{ $t('user.loan.detail.schedule.late-fee') }}
            </b-col>
            <b-col>{{ $n(lateFee, 'currency') }}</b-col>
          </b-row>
        </div>
      </b-collapse>
    </b-col>
...
export default {
  ...
  computed: {
    modalId: function getModalId() {
      return `${this.id}${this.mode}${this.sequence}`;
    },
  },
};
</script>
